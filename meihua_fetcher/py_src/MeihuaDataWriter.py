...
class MeihuaDataWriter:
  ...
  def isNumberValue(self, key):
    return key.find("ID") >= 0 or key == "Repeat" or key == "BrandCode"
  ...
  def getInsertTableSql(self, keywordId, results, adtype, keys):
    #pprint(result)
    valueList = []
    keyNames = ['`KeywordId`']
    for key in keys:
  ...
  def insertToTable(self, layer, keywordId, results, adtype):
    sql = self.getInsertTableSql(keywordId, results, adtype, results[0].keys())
    print "===== Inserting: ", sql
    layer.excute(sql, False, True)
